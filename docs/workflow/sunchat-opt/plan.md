...
状态: **已完成**。产出：S1 fakes 离线守卫；S2 KB 全管道；S3 记忆修断；S4 对话降本；S5 SSE+前端；S6 会话真操作；S7 认证诚实化；S8 Agent 正式化；S9 仓库卫生+json_parser 收敛+文档回填。

全量单测：`python -m pytest tests/ -q` → **97 passed, 1 skipped, 3.7s**（新增 ≈44 条用例；修复前无网络分支直连真实模型挂死）。

验收对照：需求 §1–§7 全部由离线单测覆盖核心路径；SSE 逐字体感、/chat/agent 端到端、上传 PDF→ready 列入实机验证清单（00-总览 §5）。

风险终态：R-1/R-2/R-3/R-5/R-6 mitigated；R-4 部分缓解（帧协议已测，前端体验待实机）。
