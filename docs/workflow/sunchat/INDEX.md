...
当前需求 ID:R-007(下一个将分配;R-005/006/007 为"全面体检后提案"三需求批次 A/B/C) |
设计基线:见 baseline.md
...
| R-005 | session 契约收口(GET 400+分页限幅+Agent 非法 id 400+死文件清理) | bugfix | active | - | 2026-09-11~ |
| R-006 | 热路径延迟优化(去重复 LLM/行情调用+轻调用短超时+/health 缓存) | iteration | planned | - | - |
