...
| FR-2 | wttr.in 实时直查:中文城市映射(41 城)+英文城市透传+无城市时 IP 定位;解析为中文结构化文本 | P1 |
...
| id | 问题 | 状态 |
|---|---|---|
| C1 | 未收录的天气描述词直接透传 wttr.in 英文原文(如 "Patchy rain nearby"),可接受还是需补翻译? | open |
| C2 | 未指明城市时用 wttr.in 服务端 IP 定位(返回服务器所在城市而非用户城市),可接受? | open |
| C3 | 天气直查失败的回退策略:回退 DDG 普通搜索(当前实现) vs 直接如实告知不可用? | open |
